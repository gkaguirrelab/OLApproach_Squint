...
% event as necessary.
for ii = 1:length(protocolParams.nTrials)
    fprintf('- Preconfiguring trial %i or %i...', ii, protocolParams.nTrials);
    
    block(ii).modulationData = modulationData(protocolParams.trialTypeOrder(ii));
    
    % Check if the 'attentionTask' flag is set. If it is, set up the task
    block(ii).attentionTask.flag = protocolParams.attentionTask;
    ...
    if block(ii).attentionTask.flag
        % Figure out how many segments there are per trial
        nSegments = block(ii).modulationData.params.trialDuration/protocolParams.attentionSegmentDuration;
        if (nSegments ~= round(nSegments))
            error('attentionSegmentDuration must evenly divide trial duration');
        end     
        segmentDuration = block(ii).modulationData.params.trialDuration/nSegments;
        ...
        actualAttentionEventDuration = block(ii).modulationData.params.timeStep*ceil((1/block(ii).modulationData.params.timeStep)*protocolParams.attentionEventDuration);
        actualAttentionMarginDuration = block(ii).modulationData.params.timeStep*ceil((1/block(ii).modulationData.params.timeStep)*protocolParams.attentionMarginDuration);
        ...
        for s = 1:nSegments
            ...
            theStartSegmentIndex = (1/block(ii).modulationData.params.timeStep)*protocolParams.attentionSegmentDuration*(s-1)+1;
            theStopSegmentIndex = (1/block(ii).modulationData.params.timeStep)*protocolParams.attentionSegmentDuration*s;
            ...
            if theCoinFlip
                ...
                theStopBlankIndex = theStartBlankIndex+(1/block(ii).modulationData.params.timeStep)*actualAttentionEventDuration-1;
                
                % Blank out the starts/stops for the trial so that the stimulus gets quite dim during the
                % attention event.
                block(ii).modulationData.modulation.starts(theStartBlankIndex:theStopBlankIndex,:) = 0;
                block(ii).modulationData.modulation.stops(theStartBlankIndex:theStopBlankIndex,:) = 1;
                
                % Say when the attention event was within the segment
                block(ii).attentionTask.segmentFlag(s) = 1;
                block(ii).attentionTask.theStartBlankIndex(s) = theStartBlankIndex;
                block(ii).attentionTask.theStopBlankIndex(s) = theStopBlankIndex;
            else
                % Say there was no attention event
                block(ii).attentionTask.segmentFlag(s) = 0;
                block(ii).attentionTask.theStartBlankIndex(s) = -1;
                block(ii).attentionTask.theStopBlankIndex(s) = -1;
            end         
        end
    end
