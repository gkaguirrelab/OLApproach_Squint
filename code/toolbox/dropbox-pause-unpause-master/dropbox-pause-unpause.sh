...
# Note: this script uses `echo` which is expected to be defined in ~/.zshenv. See https://github.com/tjluoma/echo for more details.
...
case "$ACTION" in
	...
	PAUSE)
				...
				if [ "$DB_PID" = "" ]
				then
						echo "Dropbox is not running, so cannot be paused"
						...
						then
								...
								kill -STOP "$DB_PID" && load_plist || echo "Attempt to kill -STOP $DB_PID failed"
	...
	CONTINUE)
				...
				then
						...
						then
								...
								kill -CONT "$DB_PID" && unload_plist || echo "Attempt to kill -CONT $DB_PID failed"
